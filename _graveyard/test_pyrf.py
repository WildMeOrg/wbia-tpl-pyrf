...
TEST_DATA_DETECT_URL = (
    'https://wildbookiarepository.azureedge.net/data/testdata_detect.zip'
)
...
def test_pyrf():
    ...
    std_gpath_list = image.resize_imagelist_to_sqrtarea(
        big_gpath_list, sqrt_area=800, output_dir=output_dir, checkexists=True
    )
    ...
    with utool.Timer('[test_pyrf] for loop detector.detect') as t1:
        if not utool.get_argflag('--skip1'):
            for ix, (img_fpath, dst_fpath) in enumerate(
                zip(std_gpath_list, dst_gpath_list)
            ):
