...
def run_detection(species, sensitivity=None):
    ...
    output_list = [
        'output/%s/%d.JPEG' % (species, i + 1) for i in range(len(test_gpath_list))
    ]
    ...
    results_iter = detector.detect(
        forest, test_gpath_list, output_gpath_list=output_list, sensitivity=sensitivity
    )
