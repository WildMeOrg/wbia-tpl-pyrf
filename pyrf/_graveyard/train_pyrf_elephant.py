...
def train_pyrf():
    ...
    for phase in range(1, boosting + 1):
        ...
        if phase < boosting:
            ...
            forest = detector.load(
                trees_path, category + '-', num_trees=(phase * num_trees)
            )
            ...
            dst_filepath_list = [
                join(detect_path, split(image_filepath)[1])
            ...
            for index, (predictions, image_filepath) in enumerate(
                ...
                accuracy, true_pos, false_pos, false_neg = image.accuracy(
