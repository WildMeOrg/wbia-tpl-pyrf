...
class Tree(object):
    ...
    def prepare(self, c=0):
        ...
        if self.left is not None and self.data[0] == -1:
            c = self.left.prepare(c)

        if self.right is not None and self.data[0] == -1:
    ...
    def output(self):
        ...
        if left is None and right is None:
            ...
            ratio = (
                1
                if self.leaf[0] == 0
                else int(round((len(self.leaf[1]) / 2) / self.leaf[0]))
            )
