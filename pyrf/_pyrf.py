...
METHODS = {}
METHODS['init'] = ([C_BOOL, C_BOOL], C_OBJ)  # verbose  # quiet
...
METHODS['free_detector'] = (
    [C_OBJ, C_BOOL, C_BOOL],  # detector  # verbose  # quiet
    None,
)

METHODS['free_forest'] = ([C_OBJ, C_BOOL, C_BOOL], None)  # forest  # verbose  # quiet
...
# =================================
class Random_Forest_Detector(object):  # NOQA
    ...
    def forest(rf, tree_path_list, **kwargs):
        ...
        """
        # Default values
        params = odict([('serial', False), ('verbose', rf.verbose), ('quiet', rf.quiet)])
        # params.update(kwargs)
        ut.update_existing(params, kwargs)

        # Data integrity
        assert len(tree_path_list) > 0, 'Must specify at least one tree path to load'
        assert all(
            [exists(tree_path) for tree_path in tree_path_list]
    ...
    def train(rf, train_pos_cpath_list, train_neg_cpath_list, trees_path, **kwargs):
        ...
        # Data integrity
        assert (
            params['chips_norm_width'] is None
            or params['chips_norm_width'] >= params['patch_width']
        ), 'Normalization width too small for patch width'
        assert (
            params['chips_norm_height'] is None
            or params['chips_norm_height'] >= params['patch_height']
        ), 'Normalization height too small for patch height'
        assert params['patch_width'] > 0, 'Patch width must be positive'
        assert params['patch_height'] > 0, 'Patch height must be positive'
        assert params['patch_density'] > 0.0, 'Patch density must be positive'
        assert (
            0.0 <= params['chips_prob_flip_horizontally']
            and params['chips_prob_flip_horizontally'] <= 1.0
        ), 'Horizontal flip probability must be between 0 and 1'
        assert (
            0.0 <= params['chips_prob_flip_vertically']
            and params['chips_prob_flip_vertically'] <= 1.0
        ), 'Vertical flip probability must be between 0 and 1'
        assert params['trees_num'] > 0, 'Number of trees must be positive'
        assert params['trees_offset'] >= 0, 'Number of trees must be non-negative'
        assert params['trees_max_depth'] > 1, 'Tree depth must be greater than 1'
        assert (
            params['trees_max_patches'] % 2 == 0 and params['trees_max_patches'] > 0
        ), 'A tree must have an even (positive) number of patches'
        assert (
            0.0 <= params['trees_prob_optimize_mode']
            and params['trees_prob_optimize_mode'] <= 1.0
        ), 'Tree optimization mode probability must be between 0 and 1 (inclusive)'
        assert all(
            [exists(train_pos_cpath) for train_pos_cpath in train_pos_cpath_list]
        ), 'At least one specified positive chip path does not exist'
        assert all(
            [exists(train_neg_cpath) for train_neg_cpath in train_neg_cpath_list]
        ), 'At least one specified positive chip path does not exist'
        # We will let the C++ code perform the patch size checks

        if not params['quiet']:
            print('[pyrf py] Caching positives into %r' % (data_path_pos,))
        train_pos_chip_filename_list = _cache_data(
            train_pos_cpath_list, data_path_pos, **params
        )

        if not params['quiet']:
            print('[pyrf py] Caching negatives into %r' % (data_path_neg,))
        train_neg_chip_filename_list = _cache_data(
            train_neg_cpath_list, data_path_neg, **params
        )
    ...
    def detect(rf, forest, input_gpath_list, **kwargs):
        ...
        # Data integrity
        assert params['mode'] >= 0, 'Detection mode must be non-negative'
        assert 0.0 <= params['sensitivity'], 'Sensitivity must be non-negative'
        assert len(params['scale_list']) > 0, 'The scale list cannot be empty'
        assert all(
            [scale > 0.0 for scale in params['scale_list']]
        ), 'All scales must be positive'
        assert params['batch_size'] > 0, 'Batch size must be positive'
        assert (
            params['nms_min_area_contour'] > 0
        ), 'Non-maximum suppression minimum contour area cannot be negative'
        assert (
            0.0 <= params['nms_min_area_overlap']
            and params['nms_min_area_overlap'] <= 1.0
        ...
        if output_gpath_list is None:
            ...
            assert len(output_gpath_list) == len(
                input_gpath_list
        ...
        output_scale_gpath_list = _cast_list_to_c(
            ensure_bytes_strings(output_scale_gpath_list), C_CHAR
        )
        ...
        for batch in ut.ProgressIter(
            range(batch_num), lbl='[pyrf py]', freq=1, invert_rate=True
        ):
            begin = time.time()
            ...
            if num_images < min(batch_size / 2, 8):
                params['serial'] = True
            # Final sanity check
            assert len(input_gpath_list_) == len(output_gpath_list_) and len(
                input_gpath_list_
            ) == len(output_scale_gpath_list_)
            ...
            try:
                ...
                ut.printex(ex)
            results_list = _extract_np_array(
                params['results_len_array'],
            ...
            if not params['quiet']:
                print(
                    '[pyrf py] Took %r seconds to compute %d images'
